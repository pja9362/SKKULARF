   ...
   const toggleHeart = () => {
    ...
    useEffect(() => {
        ...
        .then((noticeArray)=> {
            ...
            console.log("res Here!!!!=>"+JSON.stringify(noticeArray));
            // console.log("fav info!!!=>"+heart[0]);
        })
      }, [])
                ...
                Object.keys(notices).map(key => 
                    ...
                    : <View style = {styles.content} key={key}>
                        {/* <Pressable onPress={()=>addFav(key)} style={{padding:5}}>
                            <Icon name="heart" size={20} color={'white'}></Icon>
                        </Pressable> */}
                        {heart?
                            <TouchableOpacity
                                key={key}
                            ...
                            >
                                {/* heart값에 따른 아이콘 변경 */}
                            <Icon name="hearto" size={20} color={'#595959'}></Icon>
                            </TouchableOpacity> 
                            :
                            <TouchableOpacity
                            key={key}
                        ...
                        >
                            {/* heart값에 따른 아이콘 변경 */}
                                <Icon name="heart" size={20} color={'#3D3D3D'}></Icon>
                            </TouchableOpacity>
                        }
                        <Text style = {{...styles.contentText, fontSize: 14, textAlign: 'center', backgroundColor: 'green', width: '30%', marginBottom: 5, padding: 3}} >{notices[key].department}</Text>
                        <Text style = {styles.contentText} >{notices[key].title}</Text>
